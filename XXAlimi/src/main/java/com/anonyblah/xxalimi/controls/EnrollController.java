...
//		return "addPage";
	...
	public String insertPage(HttpServletRequest request, Model model) throws IllegalArgumentException, FeedException, IOException{
		
		String feedUrl = (String)request.getParameter("feedUrl");
		RSSFeedParser parser = new RSSFeedParser(feedUrl);
		
		SyndFeed feed = parser.readFeed();
		FeedList.feedList.add(feed);
		//feedDao.save(feed);
